...
//
// DO NOT FRORGET TO READ "ATTENTION HERE" at the bottom of this long commentars :)
//
//
//	- MATERIALS
//
// 		- 1x Teensy 2.0/Arduino Mega Board
//				(www.pjrc.com)
...
//			- Lots of Wire
//
//      /!\ /!\ /!\ IF YOU'RE USING AN ARDUINO, SKIP THIS AND GO TO CONNECTION LIST FOR ARDUINO/!\ /!\ /!\
...
//
//
//	- CONNECTION LIST FOR TEENSY (SN76489s ARE INDEXED STARTING AT PIN 1)
//
//		- Teensy Vcc
//			- Breadboard +5V Bus
...
//			- Audio Jack #4 Tip
//
//	- CONNECTION LIST FOR ARDUINO (SN76489s ARE INDEXED STARTING AT PIN 1)
//
//		- Arduino Vcc
//			- Breadboard +5V Bus
//				- SN76489 #1-4 PIN 16 (Vcc)
...
//				- 75k Resistor #1-4 Leg 1
//				- Oscillator GND (bottom left with sharp edge on top left)
//              - Arduino Digital pins to SN76489 #1-4 (PIN 10,PIN 11,PIN 12,PIN 13,PIN 15,PIN 1,PIN 2,PIN 3) D0-D7 pins (please check PORT MANIPULATION and DATA PORT SETUP)
//                              - If using Arduino UNO, Digital pin 0 to D0, Digital pin 1 to D1, etc...
//                              - If using Arduino MEGA, Digital pin 37 to D0, Digital pin 36 to D1, etc...
//              - Arduino Digital pins to SN76489 #1-4 !WE (PIN 5) (please check WRITE ENABLE PIN SETUP)
//                              - If using Arduino UNO, Digital pins 8, 9, 10 and 11
//                              - If using Arduino MEGA, Digital pins 22, 23, 24 and 25
//		- Oscillator Signal Out (bottom right with sharp edge on top left)
//			- SN76489 #1-4 PIN 14 (CLOCK)
//		- SN76489 #1 PIN 7 (SND OUT)
//			- 10uF Electrolytic Capacitor #1 Positive Leg
...
//                             /!\ /!\ /!\ ATTENTION HERE /!\ /!\ /!\
//=================================================================================================//
// This is a modified version of the original Brian Peters code for using Arduino MEGA that I've made.
// It uses a MIDI library to add MIDI communication through a MIDI 5 pin DIN connector since is not 
// possible to emulate an USB MIDI device as Teensy. I've found, that, I can't figure out how to do
// the porting with MIDI for Arduino UNO, because it only has the PORTD available, which has RX 
// function which is needed for MIDI communication, so, you can't use PORTD to send data to the PSG's
// at the same time as you do MIDI communication. I've noticed about this when I almost finished 
// writing the comments, so you will find commented lines to use UNO that I left in case you won't
// midi and in case you want to make it to work without MIDI, but you won't be able to use these lines
// as I said before that RX pin is used for MIDI communication, I apologise if this causes any confusion.
//
//
// DISCONNECT RX if you're going to try this in Arduino UNO, since it's included in PORTD and it can
// cause failures. In Arduino Mega is always a good idea to program arduino before wiring up everything,
// disconnect the RX from the MIDI Input hardware, because you could also have trouble with that.
//
// Also, you will see a lot of commented lines, some of them contain the original code for Teensy just
// to make it easier to compare. There are other important code lines to setup the port. 
//
// I didn't draw any schematic yet, but if you follow the pin connection at the top and at
// comments, I've explained them depending on port manipulation and write enable's pins.
// ( Go to PORT MANIPULATION SETUP and WRITE ENABLE PIN SETUP, DATA PORT SETUP for more information. )
//
// About MIDI Input schematic, I recommend the following tutorial that is easy to follow for any begginer:
// http://www.notesandvolts.com/2015/02/midi-and-arduino-build-midi-input.html
//
// Remember this needs to add the Arduino MIDI libray, whose link is at the MIDI.h include.
// 
//
/////////////////////////////////////////////////////////////////////////////////////////////////////
// The main code belongs to Brian Peters, who made this great synth possible, you should thank him
// for his amazing work. Also I personally recomend to check out his chiptunes and other works he makes. 
// His webpage link is at the top. Original Instructable article:
// http://www.instructables.com/id/Quad-SN76489-Synth/
//
// I hope you have good luck with this post, I'll test it soon as I get the parts I need for it.
//
// - Regards, FuzzX
/////////////////////////////////////////////////////////////////////////////////////////////////////
//
// /!\ /!\ /!\ /!\ /!\THIS ARDUINO PORT IS NOT TESTED BY ME YET SO IT MAY NOT WORK/!\ /!\ /!\ /!\ /!\ 
//=================================================================================================//
...
// Arduino midi library Arduino MIDI Library v4.2
...
// This works for newest Arduino IDE 1.6.1. If you have problems with older versions replace this with
// the following code line:
// PROGMEM prog_uchar sampleData[5338] =
const unsigned char sampleData[5338] PROGMEM = {
...
void setup() {
        ...
        /////////////////////////////////////////////////////////
        //  TEENSY usb midi code from original version
        //
        ...
        MIDI.begin(MIDI_CHANNEL_OMNI); // Initialize the Arduino Midi Library to listen all channels at the same time  (MIDI_CHANNEL_OMNI)
	...
	MIDI.setHandlePitchBend(OnPitchChange);
        ...
        /////////////////////////////////////////////////////////
        // Teensy PORTB Manipulation
 	// Sets all of PORTB as outputs (Pins 0,1,2,3,13,14,15,4)
	// Connected to D0-D7 on SN76489
        ...
        /////////////////////////////////////////////////////////
        // Arduino UNO Port Manipulation (REMEMBER THAT IT WON'T WORK WITH MIDI, READ "ATTENTION HERE")
        //
        // ATTENTION!!
        // Arduino UNO PORTB can't be used because of pins 6 & 7 used for the crystal so you have to use PORTD.
        // Pins 0 & 1 are RX and TX, so DON'T connect them while uploading code to your arduino.
        /////////////////////////////////////////////////////////
        
        // Sets all of Arduino UNO PORTD as outputs (Digital Pins 0,1,2,3,4,5,6,7)
        // Connected to D0-D7 on SN76489 -> Digital pin 0 to D0, Digital pin 1 to D1, etc...
        // Also check DATA PORT SETUP to match the PORT you need to use.
        // Erase the following "//" in front of DDRD if you use Arduino UNO, leave/put them if you're using MEGA
	//DDRD = DDRD | B11111111; // set the direction for PORTD

        /////////////////////////////////////////////////////////
        // Arduino MEGA Port Manipulation 
        //
        // I suggest to use PORTC because other ports have some useful functions
        /////////////////////////////////////////////////////////
        
        // Sets all of Arduino MEGA PORTC as outputs (Digital Pins 37,36,35,34,33,32,31,30)
        // Connected to D0-D7 on SN76489 -> Digital pin 37 to D0, Digital pin 36 to D1, etc...
        // Also check DATA PORT SETUP to match the PORT you need to use.
        // Erase the following "//" in front of DDRC if you use Arduino MEGA, leave/put them if you're using UNO
	DDRC = DDRC | B11111111; // set the direction for PORTC
        
        //////////////////////////////////////////
        //  WRITE ENABLE PIN SETUP
        //////////////////////////////////////////
        
        // Teensy Write Enable Pins were 16, 17, 18 and 19
        // For Arduino UNO I suggest to use digital pins 8, 9, 10 and 11
        // For Arduino MEGA I suggest to use digital pins 22, 23, 24 and 25
	...
	// LED Pin was 11 for Teensy 2.0, for Arduino UNO and MEGA is led 13
	pinMode(13, OUTPUT);
...
//     Teensy code to read all midi channels
//	usbMIDI.read(); 
...
void writeDataToSN76489(byte data, byte chipWriteEnablePin) {
	...
	chipWriteEnablePin = chipWriteEnablePin + 16;
        ...
        //Teensy data in PORTB
	//PORTB = data;

        //For Arduino UNO use
        //PORTD = data;
        
        //For Arduino MEGA use
        PORTC = data;
